...
const ALLOW = {
  ...
  "app/globals.css::css-raw-color::.dot-field — mask-image: hex #000":
    "A MASK IS AN ALPHA CHANNEL, NOT A COLOUR. #000 in mask-image means fully opaque; the mask never paints and the value could be any colour at all without changing a pixel. Rule 3 is right to be literal about hex, and this is the one shape where a hex carries no colour information — recorded here rather than special-cased in the matcher, which would then have to know what a mask is.",
  "app/globals.css::css-off-grid:::root — --shimmer-blur: 2px":
    "A BLUR RADIUS IS NOT A LENGTH ON THE GRID. Rule 2 governs layout — positions and sizes that have to line up with other things. This value lines up with nothing: it is how far a 0.7px rim of light bleeds, and the grid has no opinion about that any more than it has one about a transition duration. Snapping it to 4px would make the rim four times thicker to satisfy a rule about spacing. Three of them because the rim width is being compared on the running site; they collapse to one when a value is chosen.",
  "app/globals.css::css-off-grid::html[data-rim=\"fine\"] — --shimmer-blur: 1px":
    "A BLUR RADIUS IS NOT A LENGTH ON THE GRID. Rule 2 governs layout — positions and sizes that have to line up with other things. This value lines up with nothing: it is how far a 0.7px rim of light bleeds, and the grid has no opinion about that any more than it has one about a transition duration. Snapping it to 4px would make the rim four times thicker to satisfy a rule about spacing. Three of them because the rim width is being compared on the running site; they collapse to one when a value is chosen.",
  "app/globals.css::css-off-grid::html[data-rim=\"hair\"] — --shimmer-blur: 0.5px":
    "A BLUR RADIUS IS NOT A LENGTH ON THE GRID. Rule 2 governs layout — positions and sizes that have to line up with other things. This value lines up with nothing: it is how far a 0.7px rim of light bleeds, and the grid has no opinion about that any more than it has one about a transition duration. Snapping it to 4px would make the rim four times thicker to satisfy a rule about spacing. Three of them because the rim width is being compared on the running site; they collapse to one when a value is chosen.",
