...
// EVERY stylesheet under app/, found rather than listed. This was
// `["app/globals.css"]` — a hand-written array — and that is now the THIRD
// time a hardcoded list has made a checker blind to the thing it exists to
// catch:
//
//   the artboard frame, pinned to a 1000px viewport, hiding 70% of a pane;
//   the legend, regenerated by a command nobody was obliged to run;
//   and this, which could not see a second stylesheet at all.
//
// A scan cannot be forgotten the way a list can. A new .css file under app/ is
// checked the moment it exists, without anyone remembering to add it.
const CSS_FILES = (function findCss(dir = "app", acc = []) {
  for (const name of readdirSync(join(ROOT_DIR, dir))) {
    const rel = `${dir}/${name}`
    if (statSync(join(ROOT_DIR, rel)).isDirectory()) findCss(rel, acc)
    else if (name.endsWith(".css")) acc.push(rel)
  }
  return acc.sort()
})()
...
const ALLOW = {
  ...
  "app/globals.css::css-off-grid:::root — --type-display-size: 50px":
    "A rung of the 1.25 ladder. DESIGN.md: type sizes have never been bound to the 4px grid - 14/21 and 20/30 predate this ramp - and binding a geometric ladder to a 4px lattice would force it back into the arithmetic ramp it replaced.",
  "app/globals.css::css-raw-color::.dot-field — -webkit-mask-image: hex #000":
    "A MASK IS AN ALPHA CHANNEL, NOT A COLOUR. #000 in mask-image means fully opaque; the mask never paints and the value could be any colour at all without changing a pixel. Rule 3 is right to be literal about hex, and this is the one shape where a hex carries no colour information — recorded here rather than special-cased in the matcher, which would then have to know what a mask is.",
  "app/globals.css::css-raw-color::.dot-field — mask-image: hex #000":
    "A MASK IS AN ALPHA CHANNEL, NOT A COLOUR. #000 in mask-image means fully opaque; the mask never paints and the value could be any colour at all without changing a pixel. Rule 3 is right to be literal about hex, and this is the one shape where a hex carries no colour information — recorded here rather than special-cased in the matcher, which would then have to know what a mask is.",
  "app/globals.css::css-off-grid:::root — --shimmer-blur: 1px":
    "A BLUR RADIUS IS NOT A LENGTH ON THE GRID. Rule 2 governs layout — positions and sizes that have to line up with other things. This value lines up with nothing: it is how far a rim of light bleeds, and the grid has no opinion about that any more than it has one about a transition duration. Snapping it to 4px would make the rim four times thicker to satisfy a rule about spacing.",
  ...
  "app/globals.css::css-bare-length:::root — --panel-shadow: 0 12px 32px rgb(0 0 0 / 0.44)":
    "A shadow's offset and blur are not spacing. --space-* encodes RELATEDNESS between two things, or the DIMENSION of one; a blur radius is neither, and writing this as `0 var(--space-12) var(--space-32)` would be using the grid to satisfy a linter rather than because 12 and 32 mean what those tokens mean. Same judgment already recorded one entry up for --bureau-focus-ring, which is the same shape: a shadow-family token whose lengths are visual rather than structural. The values are on the 4px grid anyway, so rule 1 has nothing to say about them; this entry is only about which token family they should have come from.",
...
const ACCENT_TOKENS = ["--bureau-accent", "--hero-glow", "--accent-rim"]
...
/* ── THE SECOND NARROWING: A MASKED RIM ────────────────────────────────────
 *
 * The shimmer on a chip paints a conic gradient in the accent and then covers
 * all but a rim of it with an opaque backdrop inset by 0.05em. The visible
 * result is about 0.7px wide — NARROWER THAN THE 2px RAIL BAR this rule
 * already permits — but the declaration is a background, so rule 6 saw a
 * surface where the render has an edge.
 *
 * Recognised the same way the wash is: structurally, never by class name, so
 * nothing can opt in by adding a selector. Three conditions, each a property
 * of what a masked rim IS:
 *
 *   1. CSS, not JSX — a component cannot mint one inline.
 *   2. The value is a gradient, and it uses --accent-rim, never
 *      --bureau-accent directly. That token exists so tuning a rim cannot move
 *      the accent that marks controls; naming it is the declaration of intent.
 *   3. The same block carries a NEGATIVE z-index. A rim is behind the thing
 *      that masks it; a surface is not.
 *
 * WHAT THIS STILL LETS THROUGH, stated rather than discovered later: a grep
 * cannot measure how much of the gradient the backdrop actually covers. Any
 * negative-z-index gradient declared through --accent-rim passes — including
 * one nothing masks. The rule narrows WHO may paint an accent, not HOW MUCH
 * shows. That is the same limitation the wash exception has, and the same
 * answer: the token makes it a reviewed decision rather than an available one.
 */
function isMaskedRim(src, idx, token) {
  if (token !== "--accent-rim") return false
  const open = src.lastIndexOf("{", idx)
  const close = src.indexOf("}", idx)
  if (open === -1 || close === -1) return false
  return /z-index\s*:\s*-\d/.test(src.slice(open, close))
}
...
function checkAccentFills(file, raw, { css }) {
  ...
  for (const prop of props) {
    ...
    while ((m = re.exec(src))) {
      ...
      if (css && gradient && isMaskedRim(src, m.index, token)) continue
      out.push({
...
.d::before { z-index: -1;
  background: radial-gradient(ellipse at top center, rgb(var(--hero-glow) / 0.2), transparent); }
`
const FIXTURE_RIM_GOOD = `
.shimmer-spark::before { z-index: -30;
  background: conic-gradient(from 225deg, transparent 0, rgb(var(--accent-rim)) 90deg, transparent 90deg); }
`
const FIXTURE_RIM_NEARMISS = `
/* 1. control accent, not the rim's own token */
.a::before { z-index: -30;
  background: conic-gradient(from 225deg, transparent 0, rgb(var(--bureau-accent)) 90deg, transparent); }
/* 2. sits above what would mask it */
.b::before { z-index: 30;
  background: conic-gradient(from 225deg, transparent 0, rgb(var(--accent-rim)) 90deg, transparent); }
/* 3. no z-index at all, so nothing says it is behind anything */
.c::before {
  background: conic-gradient(from 225deg, transparent 0, rgb(var(--accent-rim)) 90deg, transparent); }
/* 4. a flat fill rather than a gradient — a surface however it is declared */
.d::before { z-index: -30; background: rgb(var(--accent-rim)); }
`
...
function selfTest() {
  ...
  // The rim narrowing gets the same treatment as the wash: prove it is a
  // NARROWING and not a hole by dropping one condition at a time.
  const rimOk = checkAccentFills("<fixture-rim-good>", FIXTURE_RIM_GOOD, { css: true })
  if (rimOk.length !== 0)
    problems.push(`control: the masked rim was flagged (${rimOk.length}) — rule 6's rim exception is not working`)
  const rimMiss = checkAccentFills("<fixture-rim-nearmiss>", FIXTURE_RIM_NEARMISS, { css: true })
  if (rimMiss.length !== 4)
    problems.push(
      `control: ${rimMiss.length}/4 near-miss rims flagged — rule 6's rim exception is a HOLE, not a narrowing. ` +
      `Each fixture drops one condition (rim token / behind / has a depth / is a gradient) and must still fire.`)
